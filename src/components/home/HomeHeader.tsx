    ...
    const changePopUpVisibility = () => {
      ...
      const handleChange = (event: { target: { value: SetStateAction<string>; }; }) => {
        ...
        const searchResults = !searchInput
            ? posts
            : posts.filter(post => 
                post.postTitle.toLowerCase().includes(searchInput.toLocaleLowerCase())
                || post.postMessage?.toLowerCase().includes(searchInput.toLocaleLowerCase())
                || post.sender.name.toLowerCase().includes(searchInput.toLocaleLowerCase())
                || post.postTarget.toLowerCase().includes(searchInput.toLocaleLowerCase())
            );
            ...
            return (
                <div className="absolute bg-white shadow-md mt-2 py-2 w-full z-10">
                    {filteredPosts.map((post, index) => (
                        <div key={index} className="px-4 py-2 text-gray-600 hover:bg-gray-200 cursor-pointer">
    ...
    return(
        <>
            ...
            <div className="relative w-full bg-white shadow-md">
                <input value={searchInput} onChange={handleChange} type="text" placeholder="Search Here" className="form-control w-full p-2" aria-describedby="basic-addon2"/>
                {renderSearchResults()}
            </div>
